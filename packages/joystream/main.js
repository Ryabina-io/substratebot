...
async function main() {
  ...
  settings.network.name = chain.toJSON().replace(/\s/g, "")
  var ss58Format = nodeProperties.ss58Format.toJSON()
  if (!ss58Format) {
    settings.network.prefix = ss58Format
  } else settings.network.prefix = 42
  settings.network.decimals = nodeProperties.tokenDecimals.toJSON()
  settings.network.token = nodeProperties.tokenSymbol.toJSON()
...
async function getAPI() {
  const nodeUri =
    process.env.NODE_URI || "wss://rome-rpc-endpoint.joystream.org:9944/"
  const provider = new WsProvider(nodeUri)
  registerJoystreamTypes()
  const api = await ApiPromise.create({ provider })
  api.registry = getTypeRegistry()
  Promise.all([
...
function getNetworkStatsMessage(priceIncluded = true, isGroup = false) {
  var result = `Joystream network Stats:\n\n`
  if (priceIncluded) {
...
async function getNetworkStats(api) {
  ...
  try {
    var token_data = await getJSON(
      `https://api.coingecko.com/api/v3/coins/joystream`
    )
  } catch (error) {
    token_data = "NA"
  }
  var validators = await api.query.session.validators()
  var validators2 = await api.query.staking.validators()
  var nominators = await api.query.staking.nominators()
  var era = await api.query.staking.currentEra()
  const totalIssuance = await api.query.balances.totalIssuance()
  const totalStake = await api.query.staking.slotStake()
  const marketcap =
    token_data != "NA"
      ? formatBalance(
  ...
  networkStats.totalIssuance = totalIssuance.toJSON()
  networkStats.totalStaked = totalStake.toJSON()
